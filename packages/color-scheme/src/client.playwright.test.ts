...
async function setupTestPage(
  page: Page,
  systemColorScheme: ColorSchemeResolve
) {
  ...
  return {
    ...
    async setConfig(config: ColorSchemeConfig): Promise<void> {
      await page.evaluate(
        (config: ColorSchemeConfig) => (window.colorSchemeApi.config = config),
        config
      );
    },
    ...
    async expectState({
      ...
      expect(actual.config, `${labelPrefix}API current config`).toEqual(
        expected.config
      );
      expect(actual.resolved, `${labelPrefix}API current resolved`).toEqual(
        expected.resolved
      );

      expect(
        actual.resolvedSystem,
        `${labelPrefix}resolved system color scheme`
      ).toBe(expected.resolvedSystem);
      expect(actual.rootCssClass, `${labelPrefix}HTML root css class`).toBe(
        expected.rootCssClass
      );
      expect(
        actual.rootColorScheme,
        `${labelPrefix}HTML root color scheme`
      ).toBe(expected.rootColorScheme);
    },
  };
}
